...
func sanitisedInput(_ input: String) -> [[Int]] {
    let lines = input.components(separatedBy: "\n")
    return lines.map { $0.components(separatedBy: " ").flatMap { Int($0) } }
}

func differenceBetweenMaxAndMin(in numbers: [Int]) -> Int {
    let ascendingNumbers = numbers.sorted(by: { firstNum, secondNum in firstNum < secondNum })

    guard let max = ascendingNumbers.last,
        let min = ascendingNumbers.first else {
            return 0
    }
...
func evenDivisionResult(in numbers: [Int]) -> Int {
    // descending because this way we know that the dividend will come before the divisor
    let descendingNumbers = numbers.sorted(by: { firstNum, secondNum in firstNum > secondNum })

    for number in descendingNumbers {
        if let divisor = descendingNumbers.first(where: { (number != $0) && (number % $0 == 0) }) {
...
// ====== PART ONE ======
func partOneChecksum(input: String) -> Int {
    let rows = sanitisedInput(input)
    let rowDiffs = rows.flatMap { row in differenceBetweenMaxAndMin(in: row) }

    return rowDiffs.reduce(0, +)
}

print("Part one checksum: \(partOneChecksum(input: inputStr))")
